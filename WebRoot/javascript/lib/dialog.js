...
Object.extend(Dialogs, {
	...
	confirm:function(s, t, y_call, n_call){
		var y = new Element('input', {value:Dialogs.Lang.logout, type:'button'}),
		    n = new Element('input', {value:Dialogs.Lang.cancel, type:'button'}),
		    c = new Dialog({
				className:'confirm',
				title: t,
				close:{link:false},
	...
	prompt:function(s,t,that, y_call, n_call){
		var passwordInput = new Element('input', {type:'password', align: 'middle',id:'passwordPrompt', name:'password', title:'password'}),
			divTag= new Element('div',{align: 'center'}),
			buttonDivTag= new Element('div',{align: 'center'}),
			y = new Element('input', {value:Dialogs.Lang.unlock, type:'button'}),
		    n = new Element('input', {value:Dialogs.Lang.reset, type:'button'}),
	    	c = new Dialog({
				className:'prompt',
				title: t,
				close:{link:false},
				...
				content:function(){
					y.observe('click', function(){
						if(Object.isFunction(y_call)) y_call(that);
						else Dialogs.close();
					});
					n.observe('click', function(){
						if(Object.isFunction(n_call)) n_call(that);
						else Dialogs.close();
					});
					passwordInput.observe('keypress', function(event){
					...
					debugger;
					divTag.appendChild(passwordInput);
					buttonDivTag.appendChild(y);
					buttonDivTag.insert('&nbsp;&nbsp;&nbsp;&nbsp;');
					buttonDivTag.appendChild(n);
					return [s,'<br/><br/>',divTag, '<br/>', buttonDivTag];
				},
				afterOpen:function(){
					y.focus();
				}
